package com.zzp.second.kill.commons.utils;

import org.apache.commons.codec.binary.Base64;

import javax.crypto.Cipher;
import java.nio.charset.StandardCharsets;
import java.security.*;
import java.security.interfaces.RSAPrivateKey;
import java.security.interfaces.RSAPublicKey;
import java.security.spec.PKCS8EncodedKeySpec;
import java.security.spec.X509EncodedKeySpec;
import java.util.HashMap;
import java.util.Map;

/**
 * 公钥，私钥
 * @author ZZP
 */
public class RsaUtils {

    private static Map<Integer, String> keyMap = new HashMap<>();

    /**
     * 随机生成密钥对
     * @throws NoSuchAlgorithmException。
     */
    public static void genKeyPair() throws NoSuchAlgorithmException {
        // KeyPairGenerator类用于生成公钥和私钥对，基于RSA算法生成对象
        var keyPairGen = KeyPairGenerator.getInstance("RSA");
        // 初始化密钥对生成器，密钥大小为96-1024位
        keyPairGen.initialize(1024,new SecureRandom());
        // 生成一个密钥对，保存在keyPair中
        var keyPair = keyPairGen.generateKeyPair();
        // 得到私钥
        var privateKey = (RSAPrivateKey) keyPair.getPrivate();
        // 得到公钥
        var publicKey = (RSAPublicKey) keyPair.getPublic();
        var publicKeyString = new String(Base64.encodeBase64(publicKey.getEncoded()));
        // 得到私钥字符串
        var privateKeyString = new String(Base64.encodeBase64((privateKey.getEncoded())));
        // 将公钥和私钥保存到Map
        /*
         * 1 表示私钥
         * 0 表示公钥
         */
        keyMap.put(0,publicKeyString);
        keyMap.put(1,privateKeyString);
    }
    /**
     * RSA公钥加密
     *
     * @param str
     *            加密字符串
     * @param publicKey
     *            公钥
     * @return 密文
     * @throws Exception
     *             加密过程中的异常信息
     */
    public static String encrypt( String str, String publicKey ) throws Exception{
        //base64编码的公钥
        var decoded = Base64.decodeBase64(publicKey);
        var pubKey = (RSAPublicKey) KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(decoded));
        //RSA加密
        var cipher = Cipher.getInstance("RSA");
        cipher.init(Cipher.ENCRYPT_MODE, pubKey);
        return Base64.encodeBase64String(cipher.doFinal(str.getBytes(StandardCharsets.UTF_8)));
    }

    /**
     * RSA私钥解密
     *
     * @param str
     *            加密字符串
     * @param privateKey
     *            私钥
     * @return 铭文
     * @throws Exception
     *             解密过程中的异常信息
     */
    public static String decrypt(String str, String privateKey) throws Exception{
        //64位解码加密后的字符串
        var inputByte = Base64.decodeBase64(str.getBytes(StandardCharsets.UTF_8));
        //base64编码的私钥
        var decoded = Base64.decodeBase64(privateKey);
        var priKey = (RSAPrivateKey) KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(decoded));
        //RSA解密
        var cipher = Cipher.getInstance("RSA");
        cipher.init(Cipher.DECRYPT_MODE, priKey);
        return new String(cipher.doFinal(inputByte));
    }

    public static void main(String[] args) throws Exception {
        // 前端加密，后端解密
        System.out.println(decrypt("R2yhKMXWaB/JrgNYpiNii722Wm0gQM1bLi2csey8TkvX4FAUg5H0hZzA2bvvi7oJuIZ/IAI6nxZXqXG8oBLrl76Lgw0zIWyKnXjRqDgAWt7w7j4hBoA5XFIL+ll89EMAFyQEALUzGGflQpNvyodV7WS+lwpqmQmUSzajvKlq2h0=",
                "MIICdgIBADANBgkqhkiG9w0BAQEFAASCAmAwggJcAgEAAoGBAICGWJnH/6gsAYAZ4GRdbQrwWmL5\n" +
                        "2AfwXh8B19KwOih+Ux68jtTbvJnc7tDYEncSbuMpMy6CslrySrXP1PbjP3q6nj4CpwXppUFtAcY4\n" +
                        "MlfCiO+1QczaN/rJIBPLPRtTGPKqJd6Kba5oFDs+ScYSFFNxgyD+vVJ3LAhWji4ZWKinAgMBAAEC\n" +
                        "gYAROnEMBCw4DdYRpuZWbdOdIMTs+ZLE1aq4O1YD7w2MugpIV3xJ+j6Y3W3B3wTyoyzsFGzA8lIY\n" +
                        "MNxk7LwecC3lA7c+wImW/7IQxNi8epuIzKHO2JsreQ4NT6vfRmsUlr6QcvLzKIxQgcvjHztj6Z8l\n" +
                        "N4Ci/03GPOYGXmvVkZrIUQJBAM1slqba4ZXieNsz2EnYCNzQa3cYCu94YC69wNa4+pB3IhQH440b\n" +
                        "OQvwE8SBkfafDDXmwVJk1JX6XO8KZE4SfP8CQQCgKveH2RgFhDv4A80FdfrZ3zsAH1Km1eo5IvlW\n" +
                        "4mXFMSbT2riI1Vs3tDpElecWnnzcnedTCVSCc0lN3Xu2sMxZAkEAsuQNjeCu8sf2V24evK/Vh/Y4\n" +
                        "n83gOAqsOAgnFtaf3Y7hrm/wScGbPDol/MqZQhvfllENqaMFPlZ49/Ikx5hyIQJAHhZUL2G5oHep\n" +
                        "qDpWjhsOh1TmyWwY45w1howqWIbo5TUJfSyOE644l2s0suOR6G7XN7410SRkiqjLcWQUtEnb6QJA\n" +
                        "BkBVrN234KShy1GG6ZUqoq7Kyi+6uKTzUS3mCO+fMMH8wduGt3vQ6SISt2Hs8ZzNtAsvqxD5EIOD\n" +
                        "CZL8QkxHQQ=="));
    }
}
